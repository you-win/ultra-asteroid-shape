...
static func publish_async(event_key, payload)->void:
	"""
	Queue the given event for async publishing. PubSub._process() MUST be
	called for this to work!
	"""
	if !subscriptions.has(event_key): return
	var listeners = subscriptions[event_key]
	var toRemove = []
...
static func unsubscribe_instant(listener, event_key = null)->void:
	"""
	Unsubscribes listener from instant event_key, or all event_keys if not
	supplied
	"""
	if event_key==null:
...
static func publish_instant(event_key, payload)->Array:
	"""
	Publish the given instant event key to all listeners and return an array
	of their responses
	"""
	var result = []
